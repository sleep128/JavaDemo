...
//        new AbortPolicy();
     ...
     * @param w the worker
     */
    //runWorker()方法里面执行了我们新建Worker对象时传进去的待执行的任务
    ...
    public void execute(Runnable command) {
        ...
        if (workerCountOf(c) < corePoolSize) {
            //如果当前工作线程数小于核心线程数，就创建一个线程放到线程池(核心线程)中，并把任务放到该线程(worker)
        ...
        if (isRunning(c) && workQueue.offer(command)) {
            int recheck = ctl.get();
            // 放入队列后再次判断线程池是否shutdown(),如果已经关闭则从队列中删除此任务，并执行拒绝策略
        ...
        //就尝试新添加工作线程来执行此次任务。如果添加失败，就走拒绝策略
